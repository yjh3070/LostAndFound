<?php
    ...
    if($reply) {
        $select_query = "select id, parent from found_comment";
        $select_result = mysqli_query($con, $select_query);
        while($select_row = mysqli_fetch_array($select_result)){
            if($select_row['parent'] == $id){
                $re_reply_id = $select_row['id'];
                $delete_re_reply_query = "delete from found_comment where id = $re_reply_id";
                $delete_result = mysqli_query($con, $delete_re_reply_query);
            }
        }
    }

    $delete_query = "delete from found_comment where id = $id";
